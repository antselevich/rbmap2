...
template <
	...
	template<class type1, class type2>
		static const key_type& Key(const pair<type1, type2>& value)
		{
			return (value.first);
		}
...
template<
	class key_type,
	class mapped_type,
	class comparator = less<key_type>,
	class allocator = allocator<pair<const key_type, mapped_type>>>
	class rbmap
	...
	template<class iterator_class>
		...
		mapped_type& operator[](key_type&& key)
		{
			...
			if (place == this->end() || comp(key, this->key(place.node())))
				place = this->insert(place, std::pair<key_type, mapped_type>(key, mapped_type())); // TODO move?
			return ((*place).second);
		}
...
template<class key_type,
	...
	public:
		...
		{
			return (base_tree::insert(forward<val_type>(value)).first);
		}

		template<class val_type> iterator insert(const_iterator place, val_type&& value)
		{
			return (base_tree::insert(place, forward<val_type>(value)));
		}
