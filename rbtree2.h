...
// Const iterator
template<class tree, class base_iterator_class = _Iterator_base0>
	...
	public:
		...
		{ // predecrement
			if (tree::isnil(my_pointer)) {
				my_pointer = tree::right(my_pointer); // must be head, is it???
			}
...
template <class rbTraits>
	...
	public:
			...
			node_ptr allocate_node(my_value_type&& value)
		{
			...
			try {
				al_elems.construct(std::addressof(this->value(node)), forward<my_value_type>(value));
			}
			catch (...) {
				this->allocate_nodes.deallocate(node, 1);
				throw;
...
template<class rbTraits>
	...
	public:
		...
		typedef typename tr1::conditional<tr1::is_same<key_type, value_type>::value, const_iterator, tree_iterator<base_class> >::type iterator;
		...
		Tree(this_tree&& right)
			: base_class(right.key_comp(), right.al_elems)
		{
			assign(forward<this_tree>(right));
		}
		...
		{
			return (search_insert_left_if_eq(this->allocate_node(forward<type_to_insert>(value)), false));
		}

		template<class type_to_insert>
			iterator insert(const_iterator _where, type_to_insert&& value)
		{
			return (insert_node_with_hint(_where, this->allocate_node(forward<type_to_insert>(value))));
		}
		...
		void erase(const key_type *begin, const key_type *end) { // WHAT ???
			while (begin != end)
				erase(*begin++);
		}
		...
		bool is_balanced(bool has_been_deleted_from) {
			...
			for (iterator it = begin(); it != end(); it++) {
				...
				if (!has_been_deleted_from && this->rank(node) - this->rank(this->left(node)) == 2 && this->rank(node) - this->rank(this->right(node)) == 2) {
					cout << "2,2-node without deletion" << endl;
					return false;
				}
			}
			return true;
		...
		string print_tree(node_ptr node, int spaces) {
				string sp = "";
