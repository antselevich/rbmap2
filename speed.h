...
void midi(int max, int some,
	...
	*rb_del_time = time;
	...
	vector<int>::iterator it_some = v.begin();
	for (int i = 0; i < some; i++)
		it_some++;

	vector<int>v2(v.begin(), it_some);
...
void n_times(idi fidi, int n, int max, int some, string i1, string e, string i2) {
	ofstream _i1(i1);
	ofstream _e(e);
	ofstream _i2(i2);
	...
	for (int i = 0; i < n; i++) {
		...
		_i1 << m_ins_time << " " << rb_ins_time << endl;
		_e << m_del_time << " " << rb_del_time << endl;
		_i2 << m_ins2_time << " " << rb_ins2_time << endl;
	}
	_i1.close();
	_e.close();
	_i2.close();
...
void trend(idi fidi, int n, int dif, int some_min, int some_max, int some_step, string i1, string e, string i2) {
	ofstream _i1(i1);
	ofstream _e(e);
	ofstream _i2(i2);
	int some = some_min;
	...
	while (some < some_max) {
		...
		_i1 << max << " " << some << " " << (double)m_ins_time_avg/(double)n << " " << (double)rb_ins_time_avg/(double)n << endl;
		_e << max << " " << some << " " << (double)m_del_time_avg/(double)n << " " << (double)rb_del_time_avg/(double)n << endl;
		_i2 << max << " " << some << " " << (double)m_ins2_time_avg/(double)n << " " << (double)rb_ins2_time_avg/(double)n << endl;		
		some += some_step;
	}
	_i1.close();
	_e.close();
